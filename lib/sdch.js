...
exports.createDictionaryOptions = function(dictionaryUrl, str) {
...
exports.clientUtils = {
  createDictionaryFromOptions: createDictionaryFromOptions,
...
function createDictionaryFromOptions(opts) {
  ...
  if (tld.getPublicSuffix(domain) === domain)
    throw new Error('Domain is a TLD');

  var dictionaryUrl = parseUrl(opts.url);
  if (!domainMatch(dictionaryUrl, domain))
    throw new Error(
      'Domain does not match the one dictionary was downloaded from');
  ...
  if (referrerHost.length === idx + domain.length) {
    ...
    if (dotIdx !== -1 && dotIdx < idx)
      throw new Error('Domain is not a postfix of the referer host');
  }
  ...
  if (opts.formatVersion && opts.formatVersion !== '1.0') {
...
function canFetchDictionary(dictionaryUrl, referringUrl) {
  ...
  if (dictionaryUrl.hostname !== referringUrl.hostname ||
    ...
    return false;
  if (dictionaryUrl.protocol !== 'http:' &&
      dictionaryUrl.protocol !== 'https:' )
    return false;
  return true;
}
...
function defaultValidation(dictionary, referringUrl) {
  return canUseDictionary(dictionary, referringUrl);
}
...
SdchEncoder.prototype.flush = function(callback) {
...
SdchEncoder.prototype._transform = function(chunk, encoding, cb) {
  ...
  var callback = function (err) {
    if (!err) {
      if (!self._prologueSent) {
        self.push(self._serverHashBuf);
        self._prologueSent = true;
      }
      cb();
    } else {
      cb(err);
  ...
  this._vcdiff.write(chunk, encoding, callback);
};

SdchEncoder.prototype._sync = function(buffer) {
  var encoded = this._vcdiff._processChunk(buffer, true, true);
...
function SdchDecoder(dicts, opts) {
  ...
  if (!this._opts.validationCallback)
    this._opts.validationCallback = exports.clientUtils.defaultValidation;
...
SdchDecoder.prototype._transform = function(chunk, encoding, cb) {
  if (this._decodeState === DecodeState.ERROR)
    return cb();
  ...
  if (this._decodeState === DecodeState.RECEIVING_HASH) {
    ...
    if (chunk.length >= rest) {
      ...
      if (serverHash[SERVER_HASH_SIZE - 1] !== '\0') {
        this._decodeState = DecodeState.ERROR;
        this.close();
        return cb(new Error('Invalid server hash'));
      }
      ...
      if (!dict) {
        this._decodeState = DecodeState.ERROR;
        this.close();
        return cb(new Error('Unknown dictionary'));
      }

      if (this._opts.validationCallback && this._opts.url &&
          !this._opts.validationCallback(dict, this._opts.url)) {
        this._decodeState = DecodeState.ERROR;
        this.close();
        cb(new Error(
            'dictionary not valid for that resource: ' + this._opts.url));
        return;
      }
  ...
  if (this._decodeState === DecodeState.STREAMING) {
    this._vcdiff.write(chunk, encoding, cb);
  } else {
    cb();
  }
};
...
SdchDecoder.prototype._sync = function(buffer) {
  ...
  if (this._opts.validationCallback && this._opts.url &&
      !this._opts.validationCallback(dict, this._opts.url))
    throw new Error('dictionary not valid for that resource' + this._opts.url);

  this._opts.dictionary = dict.data;
  this._vcdiff = vcdiff.createVcdiffDecoder(this._opts);
  buffer = buffer.slice(SERVER_HASH_SIZE);

  return this._vcdiff._processChunk(buffer, true, true);
};

function vcdiffShim(sdchStream) {
  sdchStream._vcdiff.on('error', onError);
  sdchStream._vcdiff.on('end', onEnd);
  sdchStream._vcdiff.on('readable', flow);
  function flow() {
    var chunk;
    while (null !== (chunk = sdchStream._vcdiff.read())) {
      if (sdchStream._prologueSent === false) {
        sdchStream.push(sdchStream._serverHashBuf);
        sdchStream._prologueSent = true;
      }
